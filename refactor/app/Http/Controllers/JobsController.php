...
use App\Interfaces\UserMetaRepositoryInterface ;
use App\Interfaces\UserLanguagesRepositoryInterface ;
...
class JobsController extends Controller
{
    ...
    function __construct(private UserRepositoryInterface $userRepository, private JobRepositoryInterface $jobRepository, private UserMetaRepositoryInterface $userMetaRepository,
                        private UserLanguagesRepositoryInterface $userLanguageRepository)
    {
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $jobs = $this->jobRepository->getTranslatorJobs($cuser->id, 'new');
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $jobs_ids = $this->jobRepository->getTranslatorJobsHistoric($cuser->id, 'historic', $pagenum);
...
//            $jobs['total'] = $totaljobs;
    ...
    {
        $user_meta = $this->userMetaRepository->where('user_id', $user_id)->first();
        ...
        $languages = $this->userLanguageRepository->where('user_id', '=', $user_id)->get();
        $userlanguage = collect($languages)->pluck('lang_id')->all();
        $gender = $user_meta->gender;
        $translator_level = $user_meta->translator_level;
        $job_ids = $this->jobRepository->getJobs($user_id, $job_type, 'pending', $userlanguage, $gender, $translator_level);
        foreach ($job_ids as $k => $v)     // checking translator town
        {
            $job = Job::find($v->id);
            $jobuserid = $job->user_id;
            $checktown = $this->jobRepository->checkTowns($jobuserid, $user_id);
